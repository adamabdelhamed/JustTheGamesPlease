...
let rx=.28,ry=-.3,dragging=false,px=0,py=0,inputX=0,inputY=0,impactDirection={x:0,y:-1};
const reset=()=>{rx=.28;ry=-.3;actor.moveTo(0,0).setVelocity(0,0).regenerate()};q("#reset").addEventListener("click",reset);
...
canvas.addEventListener("pointermove",event=>{if(dragging){ry+=(event.clientX-px)*.008;rx+=(event.clientY-py)*.008;px=event.clientX;py=event.clientY;auto.checked=false}});
...
const render=(now:number)=>{const dt=Math.min(.04,(now-last)/1000);last=now;rx+=inputY*dt*2.2;ry+=inputX*dt*2.2;actor.shape={...currentShape(),depth:+depth.value/100};actor.scale=+zoom.value/100;actor.setVelocity(+velocityX.value/100,+velocityY.value/100).update(dt);
let ax=rx,ay=ry,az=0;if(auto.checked){const wave=Math.sin(now/700)*.24;if(actor.shape.rock==="pitch")ax+=wave;if(actor.shape.rock==="roll")az=wave;if(actor.shape.rock==="yaw"||actor.shape.rock==="orbit")ay+=wave}
...
title.textContent=actor.shape.name;readout.textContent=`${actor.shape.family.toUpperCase()} · ${actor.shape.rock} · position ${actor.x.toFixed(2)}, ${actor.y.toFixed(2)}`;
