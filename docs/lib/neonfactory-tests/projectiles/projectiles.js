...
function mesh(instance) {
  ...
  const entranceOffset = (point, z, index) => {
    ...
    const radius2 = (instance.entranceMagnitude ?? instance.explodeMagnitude ?? 0.55) * (1 - entranceEase) * (0.35 + random * 0.75);
    return [Math.cos(angle) * radius2, Math.sin(angle) * radius2, (random - 0.5) * radius2 * 0.55];
  };
...
function edgeMesh(instance) {
  ...
  const explode = (a, b, index) => {
    ...
    const dx = mx / length2 * drift * speed2 * 0.35;
    const dy = my / length2 * progress * speed2 + progress * progress * 0.18 - fallProgress * fallProgress * 0.55;
  ...
  shape2.points.forEach((point, index) => {
    ...
    for (let segment = 0; segment < segmentCount; segment++) {
      const length2 = tuning.branchLengthMin + random(seed + 10 + segment) * Math.max(0, tuning.branchLengthMax - tuning.branchLengthMin);
...
var NeonTopDownSceneRenderer = class _NeonTopDownSceneRenderer {
  ...
  render(scene, timeSeconds = performance.now() / 1e3) {
    ...
    if (shapes.length) this.#shapes.render(shapes.map((shape2) => ({
      ...
      scale: (shape2.height ?? shape2.size) / this.#height * 2.5,
      scaleX: (shape2.scaleX ?? 1) * ((shape2.width ?? shape2.size) / (shape2.height ?? shape2.size))
    })), true, target);
...
var rotationForScreenForwardVector = (x, y) => {
  const length2 = Math.hypot(x, y) || 1;
  return Math.atan2(x / length2, -y / length2);
};
var NeonProjectile = class {
  ...
  primitives() {
    ...
    const speed2 = Math.hypot(this.velocityX, this.velocityY) || 1;
    const directionX = this.velocityX / speed2;
    const directionY = this.velocityY / speed2;
