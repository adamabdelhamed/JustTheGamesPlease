...
function createEnemyActor(enemyId) {
  ...
  const actor2 = new NeonShapeActor({
    shape,
  ...
  return actor2.enter(entrance.durationSeconds, entrance.scatterMagnitude);
}
...
var actor = createEnemyActor("basicOrb");
var effects = [];
...
try {
  ...
  const render = (now) => {
    ...
    actor.update(delta);
    actor.color = color.value || neonPalette[orbFamily.members[selectedType()].baseColor];
    const shapes = effects.length && effects[0].age < 0.09 ? [actorInTopDownScene(actor, 450, 350, 18 * orbFamily.members[selectedType()].columnSpan, { color: actor.color, opacity: 1 - effects[0].age / 0.09 })] : [];
