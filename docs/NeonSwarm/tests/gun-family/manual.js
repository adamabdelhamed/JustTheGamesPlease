...
try {
  ...
  const syncLevelOptions = () => {
    ...
    levelSelect.replaceChildren(...gun.levels.map((level2) => new Option(String(level2.level), String(level2.level))));
    const validLevel = gun.levels.some((level2) => level2.level === previousLevel) ? previousLevel : gun.levels[0].level;
  ...
  const updateReadout = () => {
    ...
    specReadout.innerHTML = [
      ...
      ["Collision", `${level2.collisionRadiusScale ?? 1}x`],
      ["Projectiles", String(level2.projectileCount)],
      ["Spread", `${level2.spreadDegrees}deg`],
  ...
  document.querySelectorAll("[data-spawn-pickup]").forEach((button) => {
    button.addEventListener("click", () => sim.spawnGunPickup({ gunId: selectedGun(), lane: Number(button.dataset.spawnPickup) }));
  });
