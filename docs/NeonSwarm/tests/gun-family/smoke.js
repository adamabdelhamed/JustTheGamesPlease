...
function evaluateGunAgainstOrb(gunId, gun, orb, levelNumber = 1) {
  const level3 = gun.levels.find((candidate) => candidate.level === levelNumber) ?? gun.levels[0];
  const distance = 540;
  ...
  let shotsFired = 0;
  const hits = [];
  const cycleMs = 1e3 / level3.fireRatePerSecond;
  for (let cycleAt = 0; cycleAt < enemyArrivalMs; cycleAt += cycleMs) {
    for (let burstIndex = 0; burstIndex < level3.burstCount; burstIndex++) {
      const firedAt = cycleAt + burstIndex * level3.burstIntervalMs;
      const travelMs = distance / level3.projectileSpeed * 1e3;
      const hitAt = firedAt + travelMs;
      if (hitAt < enemyArrivalMs) {
        const currentLaneProjectiles = gun.shotPattern === "pairedSpread" ? level3.projectileCount : Math.max(1, level3.projectileCount);
  ...
  for (const hitAt of hits) {
    enemyHealth -= level3.damage;
    damageDealt += level3.damage;
    if (enemyHealth <= 0) {
      killTimeMs = hitAt;
      break;
    }
...
function updateDetails() {
  ...
  simDetails.innerHTML = `
    <dt>Weapon</dt><dd>${gun.label} (Lvl ${level3.level})</dd>
    <dt>Fire Rate</dt><dd>${level3.fireRatePerSecond}/s</dd>
    <dt>Damage</dt><dd>${level3.damage}</dd>
    <dt>Speed</dt><dd>${level3.projectileSpeed} px/s</dd>
